IF OBJECT_ID('@resultsDatabaseSchema.@comparator_cohort_table', 'U') IS NOT NULL 
DROP TABLE @resultsDatabaseSchema.@comparator_cohort_table;
CREATE TABLE @resultsDatabaseSchema.@comparator_cohort_table 
( 
...
( 
  select concept_id from @cdmDatabaseSchema.CONCEPT where concept_id in (316866,4322024,42709887)
...
LEFT JOIN
(
  select concept_id from @cdmDatabaseSchema.CONCEPT where concept_id in (4167493)
UNION  select c.concept_id
  from @cdmDatabaseSchema.CONCEPT c
  join @cdmDatabaseSchema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
DELETE FROM @resultsDatabaseSchema.@comparator_cohort_table where cohort_definition_id = @target_cohort_id;
INSERT INTO @resultsDatabaseSchema.@comparator_cohort_table (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
